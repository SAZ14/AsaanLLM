"""Local LLM client for the ATM agent.

Mirrors app/agents/loans/llm.py in loan-agent — this is the SAME fine-tuned
model (trained on merged_finetune_train.jsonl, loan + ATM tasks together),
just called from the ATM side. Points at any OpenAI-compatible server —
Ollama by default.
...
Few-shot defaults to 0: the model is already fine-tuned on this exact prompt
format, so examples only add latency. Set ATM_FEWSHOT=4 when pointing at a
base (non-fine-tuned) model.

Env:
  ATM_LLM_BASE_URL  default http://100.73.57.125:11434/v1
  ATM_LLM_MODEL     default atm-loans-qwen3-14b
  ATM_LLM_API_KEY   default "ollama" (Ollama ignores it)
  ATM_LLM_TIMEOUT   seconds before giving up on a generation (default 120)
  ATM_FEWSHOT       few-shot examples to prepend (default 0)
"""
...
FEWSHOT_PATH = Path(__file__).resolve().parents[2] / "data" / "atm" / "fewshot.jsonl"

_client = None


def get_base_url() -> str:
    return os.environ.get("ATM_LLM_BASE_URL", "http://100.73.57.125:11434/v1")


def get_model() -> str:
    return os.environ.get("ATM_LLM_MODEL", "atm-loans-qwen3-14b")


def get_timeout() -> float:
    try:
        return float(os.environ.get("ATM_LLM_TIMEOUT", "120"))
    except ValueError:
        return 120.0
...
def fewshot_count() -> int:
    try:
        return int(os.environ.get("ATM_FEWSHOT", "0"))
    except ValueError:
        return 0
...
def get_client():
    ...
    if _client is None:
        ...
        _client = OpenAI(
            api_key=os.environ.get("ATM_LLM_API_KEY", "ollama"),
